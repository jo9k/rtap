...
handles.jvx_struct.properties.technology.id_input_files_lengths = -1;
handles.jvx_struct.properties.technology.id_input_files_rates = -1;
...
handles.jvx_struct.properties.node.offline_node.id_hookup_object_name = -1;
handles.jvx_struct.properties.node.offline_node.id_hookup_function_name = -1;
...
if(a)
    ...
    % Check state of host
    if(info.STATE_INT32 == id_check_active)
        ...
        [handles.jvx_struct.properties.technology.state, handles.jvx_struct.properties.technology.id] = jvxJvxHost.jvx_find_state_components(handles, info.COMPONENTS.AUDIO_TECHNOLOGY);
        
        % Obtain state of active node component
        bset_feature_hookup_call = jvxJvxHost.jvx_lookup_type_id__name(handles.jvx_struct.tables.feature_class_bitsets, 'JVX_FEATURE_CLASS_HOOKUP_CALL');
        [handles.jvx_struct.properties.node.state, handles.jvx_struct.properties.node.id, handles.jvx_struct.properties.node.feature_hookup_audio_node] = jvxJvxHost.jvx_find_state_components(handles, info.COMPONENTS.AUDIO_NODE, bset_feature_hookup_call);
        
        [handles.jvx_struct.properties.node.state, handles.jvx_struct.properties.node.id] = jvxJvxHost.jvx_find_state_components(handles, info.COMPONENTS.AUDIO_NODE);
        handles.jvx_struct.properties.nodes = jvxJvxHost.jvx_find_names_components(info.COMPONENTS.AUDIO_NODE);
        % If device is active, it might be started
                        
        if(handles.jvx_struct.properties.node.state == id_check_active)
        ...
        if(handles.jvx_struct.properties.technology.state >= id_check_selected)
            ...
            if(a)
                ...
                [handles.jvx_struct.properties.technology.id_input_files, handles.jvx_struct.properties.technology.cat_input_files, handles.jvx_struct.properties.technology.tp_input_files] = jvxJvxHost.jvx_find_property_id_descriptor(props_tech, 'JVX_GENW_INPUT_FILES');
                ...
                [handles.jvx_struct.properties.technology.id_output_files, handles.jvx_struct.properties.technology.cat_output_files, handles.jvx_struct.properties.technology.tp_output_files] = jvxJvxHost.jvx_find_property_id_descriptor(props_tech, 'JVX_GENW_OUTPUT_FILES');
                
                % Check state greater equal active
                if(handles.jvx_struct.properties.technology.state >= id_check_active)
                    
                    % For sub ui
                    state_technology_active_higher = true;

                    % Get the names of all devices if available
                    handles.jvx_struct.properties.devices = jvxJvxHost.jvx_find_names_components(info.COMPONENTS.AUDIO_DEVICE);
                                        
                    % Next, check device
                    [handles.jvx_struct.properties.device.state, handles.jvx_struct.properties.device.id] = jvxJvxHost.jvx_find_state_components(handles, info.COMPONENTS.AUDIO_DEVICE);
...
if(handles.jvx_struct.properties.node.feature_hookup_audio_node)
       ...
       if(a)
           ...
           [handles.jvx_struct.properties.node.offline_node.id_hookup_object_name, handles.jvx_struct.properties.node.offline_node.cat_hookup_object_name, handles.jvx_struct.properties.node.offline_node.tp_hookup_object_name] = jvxJvxHost.jvx_find_property_id_descriptor(props_node, 'JVX_AUN_HOOKUP_OBJECT');
           [handles.jvx_struct.properties.node.offline_node.id_hookup_function_name, handles.jvx_struct.properties.node.offline_node.cat_hookup_function_name, handles.jvx_struct.properties.node.offline_node.tp_hookup_function_name] = jvxJvxHost.jvx_find_property_id_descriptor(props_node, 'JVX_AUN_HOOKUP_TRIGGER');
       end
