% Matlab file for accessing (get) property spectrumEstimationWinArgA
% Property related to entry in property description file unknown, starting in line -1
function [out] = get_audio_property_node_spectrumEstimationWinArgA()
	out = [];
	global jvx_host_call_global;
	try
		[p0 p1] = jvx_host_call_global('get_property_descriptor', 'JVX_COMPONENT_AUDIO_NODE', 'spectrumEstimationWinArgA');
		if(~p0)
