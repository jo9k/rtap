% Matlab file for accessing (get) property beampatternSyncNotify
% Property related to entry in property description file unknown, starting in line -1
function [out] = get_audio_property_node_beampatternSyncNotify()
	out = [];
	global jvx_host_call_global;
	try
		[p0 p1] = jvx_host_call_global('get_property_descriptor', 'JVX_COMPONENT_AUDIO_NODE', 'beampatternSyncNotify');
		if(~p0)
