...
#ifndef __PLATFORM_H__IOS__
#define __PLATFORM_H__IOS__
...
{
  int res = gettimeofday(&refData.start, NULL);
  assert(res == 0);
}
...
static inline jvxInt64 JVX_GET_TICKCOUNT_US_GET(jvxTimeStampData& refData)
{
	...
	int res = gettimeofday(&tpNew, NULL);
	assert(res == 0);

	deltaSec = tpNew.tv_sec - refData.start.tv_sec;
	deltaUSec = tpNew.tv_usec - refData.start.tv_usec;

	time_us  = deltaSec * 1000000 + deltaUSec;
	return(time_us);
}
...
static inline int JVX_WAIT_FOR_NOTIFICATION_MS(JVX_NOTIFY_HANDLE& hdl, int timeoutmsec)
		...
		int res = gettimeofday(&tp, NULL);
		long sec = tp.tv_sec;
		int msec = tp.tv_usec / 1000 + timeoutmsec;
...
#define JVX_GETFILENAMEDIR(fData) fData->d_name

#define JVX_FINDCLOSE( a) closedir(a)

#define JVX_CHDIR(a) chdir(a)

#define JVX_REMOVE_FILE_RETURN int
#define JVX_REMOVE_FILE_POS 0
#define JVX_REMOVE_FILE(fName) remove(fName)
