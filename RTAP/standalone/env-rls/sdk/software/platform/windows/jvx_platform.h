...
static inline jvxInt64 JVX_GET_TICKCOUNT_US_ABS()
{
	LARGE_INTEGER freq, time;
	QueryPerformanceFrequency(&freq);
	QueryPerformanceCounter(&time);
...
#define JVX_FINDFIRSTFILE_S(path, handle) FindFirstFile(path, &handle )
