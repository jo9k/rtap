...
{
	theDescr->allowStateMask = 0;
	theDescr->allowThreadingMask = 0;
	theDescr->category = JVX_PROPERTY_CATEGORY_UNKNOWN;
	theDescr->decHtTp = JVX_PROPERTY_DECODER_NONE;
	theDescr->propDescription = "Unknown";
	theDescr->objDescription = "Unknown";
	theDescr->format = JVX_DATAFORMAT_NONE;
	theDescr->inPropIdx = -1;
	theDescr->hdlIdx = -1;
	theDescr->num = 0;
	theDescr->accessType = JVX_PROPERTY_ACCESS_FULL_READ_AND_WRITE;
};
...
jvxErrorType static_initData(jvxHandle* data, jvxSize sz, jvxRealtimePrivateMemoryDecoderEnum decodeHint)
{
	...
	case JVX_REALTIME_PRIVATE_MEMORY_LINEAR_PLOT_ITEM:
		...
		theLinearPlotPropItem->idPropX = -1;
	...
	case JVX_REALTIME_PRIVATE_MEMORY_LINEAR_SPLOT_SINGLE_DIAG_ITEM:
		...
		theLinearCircPlotPropItem->idChannelX = -1;
	...
	case JVX_REALTIME_PRIVATE_MEMORY_PROPERTY_ITEM:
		...
		thePropertyPropItem->selection = -1;
		break;
	default:
		break;
	}
									 ...
									 jvxInt16 itemId)
{
	...
	case JVX_REALTIME_PRIVATE_MEMORY_LINEAR_PLOT_GROUP:
		...
		if(sz == sizeof(jvxRealtimeViewerPropertyPlotGroup))
		{
			...
			{
				HjvxConfigProcessor_readEntry_assignmentData(theReader, datTmp, JVX_RTV_LINPLOT_XMAX, &thePropLinPlotGroup->xmax);
				HjvxConfigProcessor_readEntry_assignmentData(theReader, datTmp, JVX_RTV_LINPLOT_XMIN, &thePropLinPlotGroup->xmin);
				HjvxConfigProcessor_readEntry_assignmentData(theReader, datTmp, JVX_RTV_LINPLOT_YMAX, &thePropLinPlotGroup->ymax);
				HjvxConfigProcessor_readEntry_assignmentData(theReader, datTmp, JVX_RTV_LINPLOT_YMIN, &thePropLinPlotGroup->ymin);
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_AUTOX, &thePropLinPlotGroup->autox);
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_AUTOY, &thePropLinPlotGroup->autoy);
	...
	case JVX_REALTIME_PRIVATE_MEMORY_LINEAR_PLOT_ITEM:
		...
		if(sz == sizeof(jvxRealtimeViewerPropertyPlotItem))
		{
			...
			if((res == JVX_NO_ERROR) && datTmp)
			{
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_COLOR_ID, &thePropLinPlotItem->color);
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_WIDTH_ID, &thePropLinPlotItem->width);
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_STYLE_ID, &thePropLinPlotItem->linestyle);
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_SHOW, &thePropLinPlotItem->show);
			}
			else
			{
				res = JVX_ERROR_UNEXPECTED;
	...
	case JVX_REALTIME_PRIVATE_MEMORY_LINEAR_SPLOT_SINGLE_DIAG_ITEM:
		...
		if(sz == sizeof(jvxRealtimeViewerPropertyCircPlotItem))
		{
			...
			if((res == JVX_NO_ERROR) && datTmp)
			{
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_CIRC_SPLOT_LINE_WIDTH, &thePropCircLinPlotItem->lineWidth);
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_CIRC_SPLOT_XCHANNEL_SELECTION, &thePropCircLinPlotItem->idChannelX);
				HjvxConfigProcessor_readEntry_assignmentInt32(theReader, datTmp, JVX_RTV_CIRC_SPLOT_BUFFER_LENGTH, &thePropCircLinPlotItem->length);
				HjvxConfigProcessor_readEntry_assignmentInt16(theReader, datTmp, JVX_RTV_CIRC_SPLOT_NUM_CHANNELS, &thePropCircLinPlotItem->num_chans);
...
jvxErrorType static_getConfiguration(IjvxConfigProcessor* theReader, jvxConfigData* writeEntriesTo, jvxHandle* privateDataItem, jvxSize sz, jvxRealtimePrivateMemoryDecoderEnum decodeId, jvxInt16 itemId)
{
	...
	case JVX_REALTIME_PRIVATE_MEMORY_LINEAR_PLOT_GROUP:
		...
		if(sz == sizeof(jvxRealtimeViewerPropertyPlotGroup))
		{
			...
			if((res == JVX_NO_ERROR) && datTmp)
			{
				HjvxConfigProcessor_writeEntry_assignmentData(theReader, datTmp, JVX_RTV_LINPLOT_XMAX, &thePropLinPlotGroup->xmax);
				HjvxConfigProcessor_writeEntry_assignmentData(theReader, datTmp, JVX_RTV_LINPLOT_XMIN, &thePropLinPlotGroup->xmin);
				HjvxConfigProcessor_writeEntry_assignmentData(theReader, datTmp, JVX_RTV_LINPLOT_YMAX, &thePropLinPlotGroup->ymax);
				HjvxConfigProcessor_writeEntry_assignmentData(theReader, datTmp, JVX_RTV_LINPLOT_YMIN, &thePropLinPlotGroup->ymin);
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_AUTOX, &thePropLinPlotGroup->autox);
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_AUTOY, &thePropLinPlotGroup->autoy);
	...
	case JVX_REALTIME_PRIVATE_MEMORY_LINEAR_PLOT_ITEM:
		...
		if(sz == sizeof(jvxRealtimeViewerPropertyPlotItem))
		{
			...
			if((res == JVX_NO_ERROR) && datTmp)
			{
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_COLOR_ID, &thePropLinPlotItem->color);
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_WIDTH_ID, &thePropLinPlotItem->width);
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_STYLE_ID, &thePropLinPlotItem->linestyle);
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_LINPLOT_SHOW, &thePropLinPlotItem->show);
				theReader->addSubsectionToSection(writeEntriesTo, datTmp);
			}
			else
			{
	...
	case JVX_REALTIME_PRIVATE_MEMORY_LINEAR_SPLOT_SINGLE_DIAG_ITEM:
		...
		if(sz == sizeof(jvxRealtimeViewerPropertyCircPlotItem))
		{
			...
			if((res == JVX_NO_ERROR) && datTmp)
			{
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_CIRC_SPLOT_LINE_WIDTH, &thePropCircLinPlotItem->lineWidth);
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_CIRC_SPLOT_XCHANNEL_SELECTION, &thePropCircLinPlotItem->idChannelX);
				HjvxConfigProcessor_writeEntry_assignmentInt32(theReader, datTmp, JVX_RTV_CIRC_SPLOT_BUFFER_LENGTH, &thePropCircLinPlotItem->length);
				HjvxConfigProcessor_writeEntry_assignmentInt16(theReader, datTmp, JVX_RTV_CIRC_SPLOT_NUM_CHANNELS, &thePropCircLinPlotItem->num_chans);
