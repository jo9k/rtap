% Matlab file for accessing (get) property inputProgress
% Property related to entry in property description file unknown, starting in line -1
function [out] = get_audio_property_dev_inputProgress()
	out = [];
	global jvx_host_call_global;
	try
		[p0 p1] = jvx_host_call_global('get_property_descriptor', 'JVX_COMPONENT_AUDIO_DEVICE', 'JVX_GENW_INFILE_PROGRESS');
		if(~p0)
