...
void CdspGui2Dlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT1, loadDisplay);
}
...
BEGIN_MESSAGE_MAP(CdspGui2Dlg, CDialog)
	...
	ON_BN_CLICKED(IDC_BUTTON1, &CdspGui2Dlg::OnBnClickedButton1)
END_MESSAGE_MAP()
...
//	fieldOpcode3.SetWindowText(txt.c_str());
	...
	for(i = 0; i < (unsigned)numberPorts; i++)
	{
...
void CdspGui2Dlg::OnNMReleasedcaptureSliderVolume(NMHDR *pNMHDR, LRESULT *pResult)
{
	...
	{
		level = (float)((float)value/1000.0);
		value = *((int*)&level);
...
CdspGui2Dlg::messageReceived(unsigned portID, long receiveID, const char* newField, unsigned amBytes)
{
	...
	case GENERIC_MESSAGE:
		...
		{
		case OPCODE_LOAD:
			val = *((float*)&newField[8]);
			valI = (int)(val*100);
			this->loadDisplay.SetWindowText((int2String(valI) + "%").c_str());
			break;
...
void CdspGui2Dlg::OnBnClickedButton1()
{
	int opcodeUser = OPCODE_LOAD;
	std::vector<int> lstParameters;

	this->printMessage("Sending Message", GENERIC_MESSAGE, OPCODE_LOAD, lstParameters);
	this->initSend(GENERIC_MESSAGE, OPCODE_LOAD, lstParameters);
}
